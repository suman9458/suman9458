# Hi 👋, I'm Suman Kumar Singh

🎓 B.Tech Computer Science Engineering Student  
💻 Aspiring Software Developer  
🚀 Preparing for Placements  

---

## 👨‍💻 About Me

- Strong foundation in Data Structures & Algorithms  
- Skilled in C++, Python, SQL, HTML, and CSS  
- Interested in Web Development and Software Engineering  
- Passionate about learning new technologies  

---

## 🔗 Coding Profiles

- LeetCode: https://leetcode.com/suman9458
- GitHub: https://github.com/suman9458

---

## 🛠 Technical Skills

- Programming: C++, Python, SQL  
- Web: HTML, CSS, JavaScript, MERN (Basics)  
- Concepts: OOP, DSA, Problem Solving  
- Tools: Git, GitHub, VS Code  

---

## 📂 Projects

### 🔹 Personal Voice Assistant (Python)
- Voice-controlled assistant using speech commands  
- Supports web search and app control  
- Improved response accuracy  

### 🔹 Online Shopping Website
- Built using HTML, CSS, JavaScript  
- Implemented cart and product categories  
- Focused on UI/UX  

---

⭐ Keep Learning | Keep Growing | Never Give Up 💪
